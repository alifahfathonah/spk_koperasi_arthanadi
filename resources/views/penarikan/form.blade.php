<form  method="POST" action="{{ url($submit_url) }}" class="form-horizontal" name="form_crud">
  ...
  <div class="form-group">
    <label class="col-lg-3 control-label">ID Penarikan *</label>
    <div class="col-lg-9">
      <input type="text" class="form-control" name="f[id_penarikan]" id="id_penarikan" value="{{ @$item->id_penarikan }}" placeholder="ID Penarikan" required="" readonly>
    </div>
  </div>
  <div class="form-group">
  ...
  <div class="form-group">
      <label class="col-lg-3 control-label">Jumlah Penarikan *</label>
      <div class="col-lg-9">
...
<script type="text/javascript">
  ...
  function calculate() {
      var saldo_awal = mask_number.currency_remove($('#saldo_awal').val());
          ...
          if(saldo_akhir <= 0)
          {
            ...
            $('#nominal').val(mask_number.currency_add(0))
            $("#saldo_akhir").val(mask_number.currency_add(saldo_awal));
          }
          else{
  ...
  $('form[name="form_crud"]').on('submit',function(e) {
    ...
    if(saldo_akhir <= 0)
    {
      alert('Tidak dapat melakukan penarikan, anda belum memiliki saldo simpanan!');
      return false;
    }
